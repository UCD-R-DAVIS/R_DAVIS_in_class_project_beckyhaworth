...
ggplot(diamonds, aes(x= carat, y= price, color=clarity)) +
  geom_point(alpha = 0.2) + theme_classic() #theme classic removes background
...
#Now I've added linear regression trendlines for each color. lm is linear model
ggplot(diamonds, aes(x= carat, y= price, color=clarity)) +
  geom_point(alpha = 0.2) + theme_classic() + 
  ggtitle("Price by Diamond Quality") + ylab("price in $") + 
  stat_smooth(method = "lm")

#Now I've instead added LOESS trendcurves for each color. loess curve takes a window of points and calculates average for sections throughout plot
ggplot(diamonds, aes(x= carat, y= price, color=clarity)) +
  geom_point(alpha = 0.2) + theme_classic() + 
  ggtitle("Price by Diamond Quality") + ylab("price in $") + 
...
#I use the colorpalette knowledge I learned from R-DAVIS every time I make a plot, and it's not an exaggeration to say that it changed my life! Here are some templates that you may use and edit in your own work.
...
#CONTINUOUS DATA
#use scale_fill_viridis_c or scale_color_viridis_c for continuous. This comes with R. _c is for continuous. 
#I set direction = -1 to reverse the direction of the colorscale so points near the top by themselves are easier to see.
...
#to bin continuous data, use the suffix "_b" instead. puts data into buckets rather than being continuous
ggplot(diamonds, aes(x= clarity, y= carat, color=price)) +
  geom_point(alpha = 0.2) + theme_classic() +
  scale_color_viridis_b(option = "C", direction = -1)

#ORDINAL (DISCRETE SEQUENTIAL) (different categories that have an inherent order, like diamond color)
...
#scale_color is for color and scale_fill is for the fill. using _color will just give you the outline
...
#here's how it looks on a barplot. People dont like bar plots
...
#QUALITATIVE CATEGORICAL. (apples and oranges)

#From RColorBrewer:
ggplot(iris, 
       aes(x= Sepal.Length, y= Petal.Length, fill = Species)) +
  geom_point(shape=24, color="black") + theme_classic() + #shape=24 is triangles
...
ggplot(iris, aes(x= Sepal.Length, y= Petal.Length, color = Species)) +
  geom_point() + theme_classic() + 
  ggtitle("Sepal and Petal Length of Three Iris Species") + 
  scale_color_manual(values = cbPalette) + #_manual and add your manual color palette
  xlab("Sepal Length in cm") + 
  ylab("Petal Length in cm")

#DIVERGING DISCRETE (to have large and small observations show up more than average ones)
#from RColorBrewer
...
fixed_gridplot <- ggdraw() + draw_plot(plot.iris, x = 0, y = 0, width = 1, height = 0.5) + # 0, 0 is bottom left corner
