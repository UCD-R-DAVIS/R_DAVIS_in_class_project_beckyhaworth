...
## column of data
head(iris[1]) # first column (base R version)
head(iris %>% select(Sepal.Length)) #(tidyverse version)
...
for(i in 1:nrow(mtcars)){ # we want to do this for every row in cars data, we could hard code but more robust to soft code
  results[i] <- mtcars$wt[i]*100 #save in vector
}

## Datafame example -----
for(i in unique(gapminder$country)){
  country_df <- gapminder[gapminder$country == i, ]
  df <- country_df %>% #save as a new df called county_df
...
# often times mean# often times iteration is paired with custom functions
head(mtcars)
